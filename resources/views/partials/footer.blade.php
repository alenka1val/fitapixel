<footer>
    <div class="footer">
        <div>
            <ul>
                <li><span>Ilkovičova 2</span></li>
                <li><span>842 16 Bratislava 4</span></li>
                <li><span>foto@fiit.stuba.sk</span></li>
            </ul>
            <!-- <img id="nav_img" src="{{asset( '/images/cewe.png' )}}" alt="CEWE" onclick="redirect('https://www.cewe.sk')" title="CEWE"> -->
        </div>
        <div>
            <!-- <h5>Organizátor fotosúťaže FIITAPIXEL:</h5> -->
            <ul>
                <li><span>Ilkovičova 2</span></li>
                <li><span>842 16 Bratislava 4</span></li>
                <li><span>foto@fiit.stuba.sk</span></li>
            </ul>
            <!-- <img class="footer-image" src="{{asset( '/images/fiit.png' )}}" alt="FIIT STU" onclick="redirect('https://www.fiit.stuba.sk')" title="FIIT STU"> -->
        </div>
        <div>
            <ul>
                <li><span><a class="footerNav" href="{{ route('gdpr') }}"  title="domov">Ochrana osobných údajov</a></span></li>
                <li><span><a class="footerNav" href="{{ route('author') }}" title="aktuality">Autorská ochrana</a></span></li>
                <li><span><a class="footerNav" href="{{ route('rss') }}" title="dokumenty">RSS kanál</a></span></li>
                <li><span><a class="footerNav" href="{{ route('faq') }}" title="fotoalbum">FAQ</a></span></li>
                <li><span><a class="footerNav" href="{{ route('links') }}" title="kontakt">Odkazy</a></span></li>
            </ul>
        </div>
        <div>
            <ul class="social-media">
                <li class="social-media">
                    <a class="facebook" href="">
                        <i class="fa fa-facebook"></i>
                    </a>
                </li>
            </ul>
        </div>
    </div>
</footer>
